...
#' dist_to_hi_titre(m)
dist_to_hi_titre <- function(map, dists, base=2, divisor=10, max_log_titre=9, min_log_titre=0){
  ...
  out$round_titre <- round_titre
  out$lessthan_titre <- lessthan_titre
  out$lessthanhack_titre <- lessthanhack_titre
  out$params <- list(base=base, divisor=divisor, max_log_titre=max_log_titre, min_log_titre=min_log_titre)
  return(out)
}
